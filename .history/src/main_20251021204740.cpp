...
void setupFilamentSensor() {
  ...
  // Debug: Read initial pin states
  delay(100);
  Serial.println("\n[SENSOR DEBUG] Initial pin readings:");
  Serial.printf("  Switch Pin %d: %s (Raw: %d)\n", 
                SENSOR_SWITCH, 
                digitalRead(SENSOR_SWITCH) == HIGH ? "HIGH" : "LOW",
                digitalRead(SENSOR_SWITCH));
  Serial.printf("  Motion Pin %d: %s (Raw: %d)\n", 
                SENSOR_MOTION,
                digitalRead(SENSOR_MOTION) == HIGH ? "HIGH" : "LOW", 
                digitalRead(SENSOR_MOTION));
  Serial.println("  Try both INPUT_PULLUP and INPUT_PULLDOWN if needed\n");
}
...
void checkFilamentSensor() {
  ...
  // Try both logic levels - adjust based on your sensor
  int switchValue = digitalRead(SENSOR_SWITCH);
  bool filamentPresent = (switchValue == LOW);  // Change to HIGH if needed
  
  // DEBUG: Log switch state changes
  static int lastSwitchValue = -1;
  if (switchValue != lastSwitchValue) {
    Serial.printf("[SENSOR DEBUG] Switch changed: %d -> %d (%s)\n", 
                  lastSwitchValue, switchValue,
                  filamentPresent ? "PRESENT" : "NOT PRESENT");
    lastSwitchValue = switchValue;
  }
...
void displayStatus() {
  ...
  bool present = (switchRaw == LOW);  // Adjust if needed
  Serial.printf("Switch Pin %d: %s (Raw: %d)\n", 
                SENSOR_SWITCH,

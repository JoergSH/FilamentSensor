...
void filamentMotionISR() {
  ...
  // Avoid using '++' on a volatile-qualified variable (deprecated); use explicit assignment
  motionPulseCount = motionPulseCount + 1;
}
...
void parseMessage(char* payload) {
  ...
  if (doc["Status"].is<JsonObject>()) {
    ...
    if (!data["Cmd"].isNull()) {
      int cmd = data["Cmd"];
      // Only log command acknowledgments, not full details
